...
import static org.jfree.chart.renderer.xy.AbstractXYItemRenderer.isPointInRect;
 ...
 *
 */
public class MyJFreeChartXYLineAndShapeRenderer extends AbstractXYItemRenderer
    ...
    //xing
    private LinkedHashMap<Integer, LinkedHashMap<Integer, Boolean>> hightLightDataMap;
    public void addHightlightDataAndPaint(int seriesIndex, int itemIndex, boolean hightlight){
        if(this.hightLightDataMap.containsKey(seriesIndex)){
            if(this.hightLightDataMap.get(seriesIndex).containsKey(itemIndex)){
                this.hightLightDataMap.get(seriesIndex).replace(itemIndex, true);
            }else{
                this.hightLightDataMap.get(seriesIndex).put(itemIndex, true);
            }
        }else{
            LinkedHashMap<Integer, Boolean> seriesHightlightMap = new LinkedHashMap();
            seriesHightlightMap.put(itemIndex, true);
            this.hightLightDataMap.put(seriesIndex, seriesHightlightMap);
        }
    }
    
    public void clearAllHightlightData(){
        this.hightLightDataMap.clear();
    }
    
    private Boolean isHightlight(int seriesIndex, int itemIndex){
      
        if(!this.hightLightDataMap.containsKey(seriesIndex) || !this.hightLightDataMap.get(seriesIndex).containsKey(itemIndex)){
            return false;
        }
        if(this.hightLightDataMap.get(seriesIndex).get(itemIndex) != null){
            return this.hightLightDataMap.get(seriesIndex).get(itemIndex);
        }
        return false;
    }
     ...
     */
    public MyJFreeChartXYLineAndShapeRenderer() {
        this(true, true);
    }
     ...
     */
    public MyJFreeChartXYLineAndShapeRenderer(boolean lines, boolean shapes) {
        ...
        //xing
        this.hightLightDataMap = new LinkedHashMap();
        this.curDataArea = new Rectangle2D.Double(10000, 10000, -10000, -10000);
    }
    ...
    private GlobalData tsData = null;
    private boolean hightLightFlag = false;
   
    public void setHightlight(boolean hightlight){
        this.hightLightFlag = hightlight;
    }
    ...
    //xing
     ...
     //xin--start
    private Rectangle2D curDataArea = null;
    public void setDataArea(Rectangle2D dataArea){
        this.curDataArea = dataArea;
    }
    public Rectangle2D getDataArea(){
        return this.curDataArea;
    }
    
//    public Point2D getPosition(XYDataset dataset,XYPlot plot, int series, int item, ValueAxis domainAxis, ValueAxis rangeAxis ){
//        double x1 = dataset.getXValue(series, item);
//        double y1 = dataset.getYValue(series, item);
//        PlotOrientation orientation = plot.getOrientation();
//        RectangleEdge xAxisLocation = plot.getDomainAxisEdge();
//        RectangleEdge yAxisLocation = plot.getRangeAxisEdge();
//        double transX1 = domainAxis.valueToJava2D(x1, this.getDataArea(), xAxisLocation);
//        double transY1 = rangeAxis.valueToJava2D(y1, this.getDataArea(), yAxisLocation);
//        Point2D p = new Point2D.Double(transX1, transY1);
//        return p;
//    }
    
    
    
//    public Shape getItemShape(XYDataset dataset,XYPlot plot, int series, int item, ValueAxis domainAxis, ValueAxis rangeAxis){
//        double x1 = dataset.getXValue(series, item);
//        double y1 = dataset.getYValue(series, item);
//        PlotOrientation orientation = plot.getOrientation();
//        RectangleEdge xAxisLocation = plot.getDomainAxisEdge();
...
//    }
    ...
    LinkedHashMap<Integer , LinkedHashMap<Integer , Shape>> itemShapes = new LinkedHashMap();  //xing
    private void addOrUpdateItemShape(int seriesIndex, int itemIndex, Shape shape){
        if(this.itemShapes.containsKey(seriesIndex)){
            if(this.itemShapes.get(seriesIndex).containsKey(itemIndex)){
                this.itemShapes.get(seriesIndex).replace(itemIndex, shape);
            }else{
                this.itemShapes.get(seriesIndex).put(itemIndex, shape);
            }
        }else{
            LinkedHashMap<Integer, Shape> tmpMap = new LinkedHashMap();
            tmpMap.put(itemIndex, shape);
            this.itemShapes.put(seriesIndex, tmpMap);
        }
    }
    
    private void removeAllItemShapes(){
        this.itemShapes = new LinkedHashMap();
    }
    
    public LinkedHashMap<Integer, LinkedHashMap<Integer, Shape>> getAllItemShapes(){
    ...
    protected void drawSecondaryPass(Graphics2D g2, XYPlot plot, 
        ...
        double x1 = dataset.getXValue(series, item);
        double y1 = dataset.getYValue(series, item);
        //System.out.println("drawSecondaryPass  hightLight: " + hightLightFlag);
        ...
        if (getItemShapeVisible(series, item)) {
            ...
            if (series == dataset.getSeriesCount() - 1 && item == 0) {             //xing
                this.removeAllItemShapes();
            }
            Rectangle2D rec = shape.getBounds2D();
            this.addOrUpdateItemShape(series, item, shape);  //xing
            ...
            if (shape.intersects(dataArea)) {
                if (getItemShapeFilled(series, item)) {
                    if (this.useFillPaint) {
                        ...
                        // Hightlight Fill Start   //xing
                        if (this.hightLightFlag) {
                            if (this.isHightlight(series, item)) {
                                Color fillPaint = (Color) getItemFillPaint(series, item);
                                Color newPaint = fillPaint.brighter();
                        ...
                        g2.setPaint(getItemPaint(series, item));
                        if (this.hightLightFlag) {  //xing
                            if (this.isHightlight(series, item)) {
                                Color fillPaint = (Color) getItemPaint(series, item);
                                Color newPaint = fillPaint.brighter().brighter();
                                g2.setPaint(newPaint);
                            }
                        }  //xing
                    }
                ...
                if (this.drawOutlines) {
                    if (getUseOutlinePaint()) {
                        ...
                        if (this.hightLightFlag) {  //xing
                            if (this.isHightlight(series, item)) {
                                Color fillPaint = (Color) getItemOutlinePaint(series, item);
                                Color newPaint = fillPaint.brighter().brighter();
                                g2.setPaint(newPaint);
                            }
                        }  // xing
    ...
    public Object clone() throws CloneNotSupportedException {
        MyJFreeChartXYLineAndShapeRenderer clone = (MyJFreeChartXYLineAndShapeRenderer) super.clone();
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof MyJFreeChartXYLineAndShapeRenderer)) {
            return false;
        }
        if (!super.equals(obj)) {
            return false;
        }
        MyJFreeChartXYLineAndShapeRenderer that = (MyJFreeChartXYLineAndShapeRenderer) obj;
